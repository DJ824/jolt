...
class SharedSpscQueue {
    ...
    void init_view(SharedRingMode mode) {
        ...
        if (mode == SharedRingMode::Create) {
            ...
            header_->reader_.read_index.store(0, std::memory_order_release);
            header_->writer_.write_index.store(0, std::memory_order_release);
...
public:
    ...
    template <typename U>
    bool enqueue(U&& item) {
        const size_t curr_tail = header_->writer_.write_index.load(std::memory_order_relaxed);
        const size_t next_tail = (curr_tail + 1) & kMask;
        if (next_tail == writer_cache_.read_index_cache_) {
            writer_cache_.read_index_cache_ = header_->reader_.read_index.load(std::memory_order_acquire);
            if (next_tail == writer_cache_.read_index_cache_) {
                return false;
            }
        }
        (*base_)[curr_tail & kMask] = std::forward<U>(item);
        header_->writer_.write_index.store(next_tail, std::memory_order_release);
        return true;
    }
    ...
    T* front() {
        const size_t curr_head = header_->reader_.read_index.load(std::memory_order_relaxed);
        if (curr_head == reader_cache_.write_index_cache_) [[unlikely]] {
            reader_cache_.write_index_cache_ = header_->writer_.write_index.load(std::memory_order_acquire);
            if (curr_head == reader_cache_.write_index_cache_) {
                return nullptr;
            }
        }
        return &(*base_)[curr_head];
    }

    void pop() {
        const size_t curr_head = header_->reader_.read_index.load(std::memory_order_relaxed);
        const size_t next_head = (curr_head + 1) & kMask;
        header_->reader_.read_index.store(next_head, std::memory_order_release);
    }

    template <typename Reader>
    bool try_pop(Reader reader) {
        T* ptr = front();
        if (!ptr) {
            return false;
        }
    ...
    bool try_dequeue(T& out) {
        ...
        header_->reader_.read_index.store((curr_head + 1) & kMask, std::memory_order_release);
        return true;
    }
    ...
    size_t drain(Fn&& fn, size_t max_items = CAPACITY - 1) {
        ...
        const size_t curr_head = header_->reader_.read_index.load(std::memory_order_relaxed);
        const size_t curr_tail = header_->writer_.write_index.load(std::memory_order_acquire);
        ...
        header_->reader_.read_index.store(idx, std::memory_order_release);
        return to_drain;
    }
    ...
    bool empty() const {
        return header_->reader_.read_index.load(std::memory_order_acquire) ==
            header_->writer_.write_index.load(std::memory_order_acquire);
    }

    size_t size() const {
        const size_t h = header_->reader_.read_index.load(std::memory_order_acquire);
        const size_t t = header_->writer_.write_index.load(std::memory_order_acquire);
        return (t - h) & kMask;
    }
